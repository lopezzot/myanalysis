...
global xsec_scanalp90_0p5_eta10
...
xsec_scanalp90_0p5_eta10 = 242.6141 #ab #from lorenzo/alp/lorenzo/xsectocyy.py
xsec_3abg90_eta10 = 5.6315e06 #ab #from polesell/work/alpbg/fcc3a/Events/3abf_90_1
luminosity = 150 #ab^-1
...
def funcs(cutDR, cutEpho):
	...
	for entry in range(0, SgnumberOfEntries):
		...
		sigmaalp = (sigmaalp**2.+(0.3*MALP)**2.)**0.5
		...
		if MALPcut < 1.5 and epho2epho1>0.2 and Sg_DeltaR<0.08:
...
def funcb(cutDR, cutEpho):
	...
	for filenumber in range(0,1000):
		...
		for entry in range(0, BgnumberOfEntries):
			...
			if MALPcut < 1.5 and epho2epho1>0.2 and Bg_DeltaR<0.08:
...
def funcssqrtb(cutDR, cutEpho):
	...
	s1 = s*xsec_scanalp90_0p5_eta10*luminosity/SgnumberOfEntries
...
def funcshisto():
	...
	histSgMALP = ROOT.TH1F("Sg_MALP", "Sg", 40, 0., 4.)
	...
	histtruemass = ROOT.TH1F("Sg_truemass", "Sg_truemass", 40, 0., 4.)
	histtruedr = ROOT.TH1F("Sg_truedr", "Sg_truedr", 100, 0.0, 1.1)
	# Loop over signal events
	for entry in range(0, SgnumberOfEntries):
		# Load selected branches with data from specified event
		SgtreeReader.ReadEntry(entry)
		...
		truemass = (truthphoton1.P4()+truthphoton2.P4()).M()
		truedr = truthphoton1.P4().DeltaR(truthphoton2.P4())
		...
		histtruemass.Fill(truemass)
		histtruedr.Fill(truedr)
		if MALPcut<1.5:
